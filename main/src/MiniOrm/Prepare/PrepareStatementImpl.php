...
use MiniOrm\Prepare\PrepareException;
use MiniOrm\Mapping\EntityMapping;

/**
 * This class uses data returned from entity mapping and prepares an insert statmeent 
...
class PrepareStatementImpl implements PrepareStatement
{
    ...
    /**
     * Builds am SQL delete statment for the given entity and mappings.
     * 
     * @todo Escape table names and column names.
    ...
    public function delete($oEntity, EntityMapping $oMapping, \PDO $sPDO)
    {
        ...
        //Escape table name and colum names.
     ...
     * 
     * @todo Property type could be determined by passing in the value and using is_bool etc.  However declaring the type in the mapping data is more exact
     * @param array $aColumn - A mapping array for a table column.
     * @return typeRetieves the column type from mapping and assign the relevant PDO TYPE CONSTANT.
     */
        ...
        if (isset($aColumn[EntityMapping::ATTR_TYPE])) {
            ...
            switch ($aColumn[EntityMapping::ATTR_TYPE]) {
                ...
                case 'lob':
                    ...
                    break;
                default:
                    //If else fails cast the type as a string which is the defaul for PDOStatement::bindParam
     ...
     * 
     * @todo Table name and column names should be filtered.
     * @param string $sTable
     * @param array $aColumns
    ...
    private function buildInsertStatement($sTable, $aColumns)
    {
        ...
        return PrepareStatement::INSERT_STATEMENT . ' ' . $sTable . ' (' . \rtrim($sColumnName, ', ') . ')' . ' VALUES (' . \rtrim($sPlaceholders, ', ') . ')';
    }
